...
- **Next Steps**: 1. Implement the queries with relationships between tables on MongoDB, figure out how to do the same for DynamoDB, then for relational databases (PostgreSQL, MySQL, and Supabase); 2. Implement the `select_table` field type in the GCE_RFC (CRUD Editor, React Function Component) listing page; 3. Implement `select_table` in the form page when in query/read-only state; Finally do the same for GCE_FLUTTER (CRUD Editor, Flutter Widgets).
- **Tickets**: GS-259
...
- **Status**: In progress (testing and documentation pending)
