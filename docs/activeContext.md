# Active Context - genericsuite superproject
...
- **Status**: In progress
- **Focus**: Generate the `CLAUDE.md` files for all GenericSuite repositories with `claude /init`, review the generated file(s), eventually complement with additional documentation in the `docs` (or `ai_docs/`) directory (e.g. security, code styling, etc.), add `.claude/settings.local.json` to `.gitignore`, and create a new files `AGENTS.md` and `GEMINI.md` with the content `@CLAUDE.md`.
...
#### 5. Unit and Integration Testing (Active)
...
#### 6. Add GCP object storage services (Active)
- **Status**: Planning phase
...
#### 7. Add Azure object storage services (Active)
- **Status**: Planning phase
